// 睿涵音乐后台管理系统 - 教学模块 JavaScript

// 模块命名空间与状态
const Teaching = window.Teaching || (window.Teaching = {});
Teaching.state = Teaching.state || {
  selectedStudents: new Set(),
  allTasks: [],
};

// 本模块引用的状态
const selectedStudents = Teaching.state.selectedStudents;
let allTasks = Teaching.state.allTasks;

// 兜底的错误提示
(function ensureGlobalAlerts() {
...
function refreshTasks() {
  fetch("/teaching/tasks/today/")
    ...
    .then((data) => {
      if (data.success) {
        allTasks = data.tasks || [];
        Teaching.state.allTasks = allTasks;
...
function renderTaskList(tasks) {
  ...
  if (!tasks || tasks.length === 0) {
    tbody.innerHTML =
      '<tr class="empty-row"><td colspan="4" class="empty-state">暂无今日任务</td></tr>';
    updateToolbarState();
    return;
  }
  ...
  let html = "";
  tasks.forEach((task) => {
    const difficultTag = task.is_difficult
      ? '<span style="color:#d32f2f;font-weight:bold;">[困难]</span>'
      : "";

    html += `
            <tr data-student-id="${task.student_id}"${
      task.is_difficult ? ' class="difficult-row"' : ""
    }>
                <td><input type="checkbox" class="row-select"></td>
                <td>
                    <div class="title-row">
                        <a href="javascript:void(0)" class="student-link" data-student-id="${
                          task.student_id
                        }">
                            ${difficultTag} ${
      task.student_name || task.student_id
    }
                        </a>
                    </div>
                </td>
...
function updateToolbarState() {
  ...
  if (selectAll) {
    const rows = Array.from(
      document.querySelectorAll("#today-task-table tbody tr")
    );
    const totalRows = rows.filter((r) => r.querySelector(".row-select")).length;
...
function getRowInputs(studentId) {
  const row = document.querySelector(
    `#today-task-table tbody tr[data-student-id="${studentId}"]`
  );
...
function submitSelectedFeedbacks() {
  ...
  fetch("/teaching/feedback/submit/", {
    ...
    .then((data) => {
      if (data.success) {
        // 标记为已提交：禁用输入、取消选中、样式标识
        ids.forEach((sid) => {
          const row = document.querySelector(
            `#today-task-table tbody tr[data-student-id="${sid}"]`
          );
          if (!row) return;
...
function showStudentDetail(studentId) {
  fetch(`/teaching/students/${studentId}/`)
    ...
    .then((data) => {
      ...
      if (
        Array.isArray(info.feedback_comments) &&
        info.feedback_comments.length > 0
      ) {
        feedbackListHtml =
          '<ul style="margin-left:16px;">' +
          info.feedback_comments.map((c) => `<li>${c || ""}</li>`).join("") +
          "</ul>";
      } else if (
        Array.isArray(info.recent_feedbacks) &&
        info.recent_feedbacks.length > 0
      ) {
        feedbackListHtml =
          '<ul style="margin-left:16px;">' +
          info.recent_feedbacks
            .map(
              (f) =>
      ...
      if (Array.isArray(info.visit_notes) && info.visit_notes.length > 0) {
        visitListHtml =
          '<ul style="margin-left:16px;">' +
          info.visit_notes.map((n) => `<li>${n || ""}</li>`).join("") +
          "</ul>";
      } else {
      ...
      if (content) {
        content.innerHTML = `
                    ...
                    <div><strong>姓名：</strong>${
                      info.student_name || info.name
                    }（${info.nickname || info.alias_name || "无别名"}）</div>
                    <div><strong>进度：</strong>${renderProgress(
                      info.progress
                    )}</div>
                    <div><strong>状态：</strong>${info.status || "—"}</div>
                    <div><strong>学习时长：</strong>${
                      info.learning_hours ?? 0
                    } 小时</div>
...
function initTeachingModule() {
  ...
  (function initTabs() {
    ...
    let activeBtn =
      ...
      tabButtons[0];
    if (activeBtn) activateTab(activeBtn.getAttribute("data-tab"));
  ...
  function renderCompletedFeedbacks(data) {
    ...
    if (list.length === 0) {
      ...
      const rows = list
        .map((item) => {
          const progressText = Array.isArray(item.progress)
            ? item.progress.join(", ")
            : item.progress || "";
          return `
                    <tr>
    ...
    if (pager) {
      ...
      pager.innerHTML = `
                <button class="btn btn-secondary" data-page="${
                  data.current_page - 1
                }" ${prevDisabled ? "disabled" : ""}>上一页</button>
                <span style="margin:0 8px;">第 ${data.current_page} / ${
        data.total_pages
      } 页</span>
                <button class="btn btn-secondary" data-page="${
                  data.current_page + 1
  ...
  // 顶部“今日任务添加”搜索框（通用 createSearchBar）
  (function initTodayAddSearch() {
    ...
    if (typeof createSearchBar === "function") {
      window.createSearchBar({
        input,
        button: btn,
        resultBox,
        placeholder: "输入学员昵称/ID 搜索添加到今日任务...",
        emptyHtml:
          '<div class="empty-state" style="padding:8px;color:#666;">未找到匹配学员</div>',
        fetchUrl: (q) =>
          `/teaching/students/search/?q=${encodeURIComponent(q)}`,
        mapItem: (item) => {
          const name = item.student_name || item.student_id;
          const id = item.student_id;
          const groupsText = Array.isArray(item.groups)
            ? item.groups.join(", ")
            : item.groups || "";
          const status = item.status || "";
          const progress = item.current_progress || "—";
          return `
                        <div class="search-row" data-student-id="${id}">
                            <div class="col name">${name}</div>
                            <div class="col groups">${groupsText}</div>
                            <div class="col status">${status}</div>
                            <div class="col progress">${progress}</div>
                            <div class="col action"><button class="btn btn-primary btn-sm" data-add-id="${id}">添加</button></div>
                        </div>
                    `;
        },
        onResultRendered: (container) => {
          container.querySelectorAll("[data-add-id]").forEach((btnEl) => {
            btnEl.addEventListener("click", async (e) => {
              const sid = e.currentTarget.getAttribute("data-add-id");
              if (!sid) return;
  ...
  (function initAnnouncementCollapse() {
    ...
    toggleBtn.addEventListener("click", (e) => {
      ...
      localStorage.setItem("announcementCollapsed", (!isHidden).toString());
