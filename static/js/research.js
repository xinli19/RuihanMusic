...
function displayStudentSuggestions(students) {
    ...
    students.forEach(student => {
        ...
        item.innerHTML = `
            <strong>${student.name}</strong> - ${student.level} - ${student.instrument}
        `;
...
function displayAssignmentHistory() {
    ...
    if (assignmentHistory.length === 0) {
        container.innerHTML = '<div class="no-records">暂无历史记录</div>';
        return;
    }
        ...
        item.innerHTML = `
            ...
            <div class="history-students">
                ${record.students.map(student => 
                    `<span class="history-student-tag">${student}</span>`
                ).join('')}
            </div>
...
function useHistoryRecord(recordId) {
    ...
    if (record) {
        ...
        // 选择教师
        const teacherBtn = document.querySelector(`[data-teacher="${record.teacher}"]`);
        if (teacherBtn) {
            teacherBtn.click();
        }
        
        // 这里应该根据历史记录填充学员列表
        alert(`已应用 ${record.teacher} 的历史分配方案`);
    }
}
